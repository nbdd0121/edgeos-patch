...
if ($vcVPN->exists('ipsec')) {
    ...
    if (@interfaces > 0) {
        foreach my $interface (@interfaces) {
            # Detect any interfaces with dhcp addresses. This controls whether dhcp_hook
            # places a call to vyatta-ipsec-dhcp.pl in the dhcp3 "ipsecd" exit hook.

            # ethernet, bridge and switch interfaces can have real or vif interfaces
            $dhcp_if += has_real_or_vif_dhcp_address($vc, 1, 1, "interfaces ethernet $interface");
            $dhcp_if += has_real_or_vif_dhcp_address($vc, 1, 1, "interfaces bridge $interface");
            $dhcp_if += has_real_or_vif_dhcp_address($vc, 1, 1, "interfaces switch $interface");

            # bonding interfaces can only have vif interfaces
            $dhcp_if += has_real_or_vif_dhcp_address($vc, 0, 1, "interfaces bonding $interface");

            # pseudo-ethernet interfaces can only have real interfaces
            $dhcp_if += has_real_or_vif_dhcp_address($vc, 1, 0, "interfaces pseudo-ethernet $interface");
    ...
    foreach my $peer (@peers) {
        ...
        if ($peer_is_v6) {
            ...
            if (not $conn_is_v6) {
                ...
                vpn_die(
                    ...
                    "$vpn_cfg_err local-address 'default' is not supported for " . 'IPv6 peers'
                ) if ($lip eq 'default');
        ...
        } elsif ($lip ne 'any' && $lip ne 'default') {
            if($dhcp_iface && !$lip) {
                # This is a case when DHCP interface was defined but no address was received yet.
                # It is not a problem because '/etc/dhcp3/dhclient-exit-hooks.d/ipsecd' hook
                # will reset VPN connection once DHCP interface receives IP address from server.
                # See dhcp_hook() implemenation for details
                print "IP address of DHCP interface $dhcp_iface is not present yet\n";
                print "Peer \"$peer\" will be automatically restarted once $dhcp_iface receives address from DHCP server\n\n";
            }
            # not 'any|default' special case.
            # check interface addresses.
            elsif (!Vyatta::Misc::is_local_address($lip)) {
                vpn_log("The local-address $lip of peer \"$peer\" has not been configured on any interfaces.\n");
                if (Vyatta::Misc::isClusterIP($vc, $lip)) {
                    ...
                    print "IPsec must be re-started after address\n";
        ...
        foreach my $tunnel (@tunnels) {
            ...
            if ($authremoteid) {
                ...
                if ($fqdn_peer) {
                    $rightid = '%any';      # Use rightid '%any' in ipsec.conf
                    $authremoteid = '%any'; # Use selector '%any' in ipsec.secrets
                }
            }
            ...
            my $lprotoport = '';
            if (defined($protocol)) {
                $lprotoport .= ($protocol eq 'all') ? '%any' : $protocol;
            }
            ...
            my $rprotoport = '';
            if (defined($protocol)) {
                $rprotoport .= ($protocol eq 'all') ? '%any' : $protocol;
            }
            ...
            if (!defined($auth_mode) || $auth_mode eq '') {
                ...
                # Aggressive ike mode is not allowed with PSK
                my $ike_group_name = $vcVPN->returnValue("ipsec site-to-site peer $peer ike-group");
                if (defined($ike_group_name)) {
                    my $ike_group_mode = $vcVPN->returnValue("ipsec ike-group $ike_group_name mode");
                    if (defined($ike_group_mode) && $ike_group_mode eq "aggressive") {
                        vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                            "$vpn_cfg_err Aggressive IKE mode is not allowed with PSK\n");
                    }
                }
                ...
                if (  ($lip eq 'any' || $lip eq 'default' ||
                    ...
                    if($index2 eq '%any') {
                        # local-address and remote-address are both dynamic
                        $genout_secrets .= "%any %any : PSK \"$psk\"\n";
                    } else {
                ...
                # Deny RSA key authentication with 0.0.0.0 peer
                if ($peer eq '0.0.0.0') {
                    vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                        "$vpn_cfg_err RSA key authentication is not allowed with 0.0.0.0 peer\n");
                }
...
# but do not start the VPN daemon
        ...
        if (is_vpn_running()) {
            if (defined($update_interval) && ($update_interval != $update_interval_orig)) {
                ...
                vpn_exec('ipsec restart >&/dev/null', 'reload changes to ipsec.conf');
            }
        } else {
...
if (defined($auto_fw_nat) && $auto_fw_nat eq 'enable') {
    ...
    for my $peer (@peers) {
        # Do not make nat-exclude for IPv6 peers
        my $peer_ip = new NetAddr::IP $peer;
        next if(defined($peer_ip) && $peer_ip->version() == 6);
...
sub has_dhcp_address {
    my ($vc, $path) = @_;
    my @if_addresses = $vc->returnValues($path . " address");
    foreach my $if_addr (@if_addresses) {
        if ($if_addr eq 'dhcp') {
            return 1;
        }
    }
...
sub has_real_or_vif_dhcp_address {
    my ($vc, $chk_real, $chk_vif, $path) = @_;
    my $ret = 0;
    if ($chk_real) {
        $ret += has_dhcp_address($vc, $path);
    }
    if ($chk_vif) {
        my $vif_path = $path . " vif";
        my @vif = $vc->listNodes($vif_path);
        foreach my $vif (@vif) {
            if (has_dhcp_address($vc, $vif_path . ' ' . $vif)) {
                $ret++;
                last;
            }
        }
