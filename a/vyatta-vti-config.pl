...
if (!$vcVPN->exists('ipsec site-to-site') ) {
    ...
    foreach my $peer (@peers) {
        ...
        if (defined($vtiPresent) && !($vtiPresent eq "") &&
            ($vtiPresent ne $tunName)) {
            $gencmds .= "sudo /sbin/ip link delete $vtiPresent type vti > /dev/null 2>&1\n";
            vtiIntf::deleteVtibyname($vtiPresent);
        }
        else {
          if (defined($vtiPresent) && ($vtiPresent eq "")) {
            if (vtiIntf::isVtibynamepresent($tunName)) {
              $gencmds .= "sudo /sbin/ip link delete $tunName type vti > /dev/null 2>&1\n";
    ...
    foreach my $peer (@peers) {
        ...
        if (!(validateType('ipv4', $peer, 'quiet') || 
              validateType('ipv6', $peer, 'quiet')) || ($peer eq '0.0.0.0')) {
            vti_die(["vpn","ipsec","site-to-site","peer",$peer],
                    "$vti_cfg_err The peer \"$peer\" is invalid, an ip address must be specified for VTIs.\n");
        }
        ...
        my $disabled = $vcIntf->exists("vti $tunName disable");
        ...
        if (defined($existingMark) && !($existingMark eq "") && defined($vtiPresent) && !($vtiPresent eq "") && ($vtiPresent eq $tunName)) {
	    $mark = $existingMark;
        } else {
...
sub cleanupVtiNotConfigured {
    ...
    for my $name (keys %$localVtibyNames) {
        ...
        $gencmds .= "sudo /sbin/ip link delete $name type vti > /dev/null 2>&1\n";
    }
}
